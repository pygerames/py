# سورس ربات مدیریت اکانت اپن شد برای اولین بار در آیرا تیم
...
#           ---         ---         ---         #
api_id = 13770094 # main api id from my.telegram.org/apps
api_hash = '663a46fc8daafa97bc0cfedef54e6ba6' # main api hash from my.telegram.org/apps
bot_token = '6741812592:6680688527:AAHXucST9zT_-ZTs6N3pDiyedLqYUvk6UpY' # main bot token from @botFather
bot_admins = [6663317718] # admin userID
#           ---         ---         ---         #
...
async def StartResponse(client, message):
    ...
    my_keyboard = [
        ...
        [InlineKeyboardButton('تنظیم زمان 🕠', callback_data='setTime'), InlineKeyboardButton('📛 لغو تمام عملیات ها', callback_data='endelhak')],
    ]
...
if data == 'addAccounts':
    step = 'getPhoneForLogin'
    my_keyboard = [
        [InlineKeyboardButton('🔙', callback_data='backToMenus')],
    ]
    await bot.edit_message_text(
        chat_id,
        message_id,
...
if data == 'accountAddedSuccessfully':
    my_keyboard = [
        [InlineKeyboardButton('افزودن اکانت ➕', callback_data='addAccounts')],
        [InlineKeyboardButton('ویو پست👁', callback_data='viewEvall')],
        [InlineKeyboardButton('🔗 برو به چنل تلگرام', url='https://t.me/balegeram')],
    ]
    await bot.edit_message_text(
        chat_id,
        message_id,
        '<b>- اکانت با موفقیت اضافه شد. ممنون از شما!</b>',
        reply_markup=InlineKeyboardMarkup(my_keyboard)
    )
...
if data == 'removeAccount':
    ...
    if myLen == 0:
        await bot.invoke(
            functions.messages.SetBotCallbackAnswer(
                ...
                cache_time=1,
                alert=True,
                message='اکانتی یافت نشد !'
            )
        )
    else:
        with open('./accounts.txt', 'w') as my_file:
            my_file.write("\n".join(await accountList()))
        try:
            await bot.send_document(
                chat_id,
                './accounts.txt',
                caption=f'تعداد کل اکانت ها: {myLen}'
            )
            os.unlink('./accounts.txt')
        except:
            pass
    ...
    if len(await accountList()) == 0:
        ...
        await bot.edit_message_text(
            chat_id,
            message_id,
            '<b>عملیات بررسی اکانت ها شروع شد ...</b>'
        )
...
@bot.on_message(filters.command(['start', 'cancel']) & filters.private & filters.user(message.from_user.id))
async def StartResponse(client, message):
    global step, tempClient, isWorking
    try:
        tempClient['client'].disconnect()
